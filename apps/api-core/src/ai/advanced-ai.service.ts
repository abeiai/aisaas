...
export class AdvancedAiService {
  ...
  async listWorkflows() {
    ...
    return workflows.map((workflow) => this.toWorkflow(workflow));
  }
  ...
  async createWorkflow(dto: CreateWorkflowDto) {
    ...
    if (steps.length === 0) {
      throw new AppException(40001, "工作流至少需要一个步骤", HttpStatus.BAD_REQUEST);
    }

    const workflow = await this.prisma.aiWorkflow.upsert({
      where: {
        slug: dto.slug.trim()
      },
      update: {
    ...
    return this.toWorkflow(workflow);
  }

  async runWorkflow(userId: string, workflowId: string, dto: RunWorkflowDto) {
    ...
    if (!workflow) {
      throw new AppException(40401, "工作流不存在或已停用", HttpStatus.NOT_FOUND);
    }

    if (workflow.steps.length === 0) {
      throw new AppException(40001, "工作流未配置步骤", HttpStatus.BAD_REQUEST);
    }
    ...
    let currentInput = dto.input.trim();

    try {
      for (const step of workflow.steps) {
        const output = this.mockWorkflowStep(step.prompt, currentInput);
        await this.prisma.aiWorkflowRunStep.create({
          data: {
            runId: run.id,
            stepId: step.id,
      ...
      const finished = await this.prisma.aiWorkflowRun.update({
        where: {
          id: run.id
        },
        data: {
      ...
      return this.toWorkflowRun(finished);
    } catch (error) {
      const failed = await this.prisma.aiWorkflowRun.update({
        where: {
          id: run.id
        },
        data: {
      ...
      return this.toWorkflowRun(failed);
    }
  }
  ...
  private mockWorkflowStep(prompt: string, input: string) {
    if (input.includes("触发失败")) {
      throw new Error("工作流步骤执行失败");
    }

    return [`步骤要求：${prompt}`, "", `输入内容：${input}`, "", "输出：已按步骤要求完成处理。"].join("\n");
  }
  ...
  private toWorkflowRun(run: {
    ...
    return {
      ...
      userId: run.userId,
      status: run.status,
      statusName: run.status === "SUCCEEDED" ? "执行成功" : run.status === "FAILED" ? "执行失败" : "执行中",
      input: run.input,
      output: run.output,
