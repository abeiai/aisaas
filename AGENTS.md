...
本项目所有本地开发、Docker 容器内部端口、服务间通信、文档示例和默认环境变量优先使用 `7340-7360` 之间的端口，不得换用 `3000`、`3002`、`3003`、`5432`、`5433`、`6379`、`6380`、`8001`、`8080`、`8443` 等其他项目常用端口。

生产公网入口例外：线上对外访问可以使用常用端口 `80/443`，但只能作为宿主机或负载均衡入口映射到容器内部的 `7340/7346`，不得改变容器内部端口和服务间通信端口。

固定端口表：

```text
7340  Docker / Nginx HTTP 容器内部统一入口
7341  Next.js Web
7342  NestJS API Core
7343  FastAPI AI Gateway
7344  本地开发 PostgreSQL
7345  本地开发 Redis
7346  Docker / Nginx HTTPS 容器内部入口
